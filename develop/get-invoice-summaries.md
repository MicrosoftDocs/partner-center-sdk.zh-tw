...
若要取出[**InvoiceSummaries**](invoice-resources.md#invoicesummaries)集合，其中包含每個貨幣類型的[**InvoiceSummary**](invoice-resources.md#invoicesummary) ：

1. 使用您的**iaggregatepartner.customers.byid （發票**）集合來呼叫**摘要**屬性。
2. 呼叫**Get （）** 方法。
3. 若要取得個別[**InvoiceSummary**](invoice-resources.md#invoicesummary)的餘額，請存取該集合成員的**BalanceAmount**屬性。
...
| 方法  | 要求 URI                                                                   |
|---------|-------------------------------------------------------------------------------|
| **獲取** | [ *{baseURL}* ](partner-center-rest-urls.md)/v1/invoices/summaries HTTP/1。1     |
...
如果成功，此方法會在回應主體中傳回[**InvoiceSummaries**](invoice-resources.md#invoicesummaries)資源。
