...
| friendlyName         | 字串                                    | 選擇性。 合作夥伴所定義之訂用帳戶的易記名稱，以協助區分。                                                                                                                                              |
...
| 訂用帳戶       | [連結](utility-resources.md#link)            | 填入完整訂閱資訊的連結。                       |
...
| status                          | 字串                              | 訂單明細專案的布建狀態。 數值包括：</br>「已滿足」：訂單的履行成功完成，使用者將能夠使用保留專案</br>「未執行」：因取消而未完成</br>"PrefulfillmentPending"：您的要求仍在處理中，履行尚未完成 |
| quantityProvisioningInformation | 清單<[QuantityProvisioningStatus](#quantityprovisioningstatus)> | 訂單明細專案的數量布建狀態資訊清單。 |
