...
| 屬性 | 類型 | 描述 |
|----------|------|-------------|
| CustomerTenantId | string | 客戶租使用者識別碼。 |
| customerName | string | 客戶名稱。 |
| subscriptionId | string | 訂用帳戶識別碼。 |
| subscriptionName | string | 訂用帳戶名稱。 |
| usageDate | string | 使用日期。 |
| resourceLocation | string | 資料中心的位置，例如西歐。 |
| meterCategory | string | 計量分類，例如 [資料管理]。 |
| meterSubcategory | string | 「計量子類別」，例如「異地多餘」。 |
| meterUnit | string | 計量單位，例如 gb 或小時。 | 
| ReservationOrderId | string | Azure VM 保留實例的保留順序。 |
| reservationId | string | 在特定 RI 訂單下的保留實例。 |
| serviceType | string | 表示虛擬機器類型。 例如，Standard_E4s_v3。 |
...
| 屬性 | 類型 | 描述 |
|----------|------|-------------|
| partnerTenantId | string | 您要為其取得間接轉銷商資料之夥伴的租使用者識別碼。 |
| id | string | 間接轉銷商識別碼。 |
| 名稱 | string | 您要為其取得間接轉銷商資料的夥伴名稱。 |
| market | string | 您想要為其取得間接轉銷商資料的合作夥伴市場。 |
...
|            id             |             string             |                                                              訂用帳戶識別碼。                                                              |
|          status           |             string             |                                          訂用帳戶狀態：「作用中」、「已暫停」或「取消布建」。                                           |
...
| keywordCountry | string | 搜尋中指定的國家/地區。 |
| detailsViewed | 布林值 | 指出是否已查看搜尋詳細資料。 |
| keywordIndustryFocus | string | 要在其中進行搜尋的產業，例如醫療保健。 |
| mpnId | string | Microsoft 合作夥伴網路（MPN）識別碼。 若為直接轉銷商，這會是合作夥伴的 MPN 識別碼。 若為間接轉銷商，這將是間接轉銷商的 MPN 識別碼。 |
| partnerMarket | string | 合作夥伴開展業務的地區設定。 |
| keywordProduct | string | 搜尋中指定的產品。 |
| referralSubmitted | 布林值 | 指出是否已提交參照。 |
| searchDate | UTC 日期時間格式的字串 | 搜尋查詢的發生日期。 |
| keywordSearchText | string | 搜尋中指定的文字。 |
...
|----------|------|-------------|
| id | string | 客戶租使用者識別碼。  |
| status | string | 指出參考是否導致客戶。  |
| customerMarket | string | 客戶執行業務的國家/地區。 |
| customerName | string | 客戶的名稱。 |
| customerOrgSize | string | 範圍，指出客戶組織中的員工數目。 例如，"10to50employees"。 |
...
| partnerMarket | string |  合作夥伴經營業務的國家/地區。 |  
